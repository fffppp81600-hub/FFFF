...
_cooldown: dict  = {}
EDIT_MODE: dict  = {}
RENAME_MODE: dict = {}    # uid -> proj_name — بانتظار العنوان الجديد لصفحة منشورة فعلاً
USER_STATS: dict = {}
PENDING_IMAGE: dict = {}
...
def _project_keyboard(proj_name: str, url: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📝 غيّر العنوان", callback_data=f"rename|{proj_name}"),
...
# ── تغيير عنوان الصفحة (Title) — مباشر بدون AI، لا يغيّر الرابط ─────
def _rename_page_title(html: str, new_title: str) -> str:
    """يبدّل محتوى <title> و og:title (إن وُجد) فقط — لا يلمس باقي الكود إطلاقاً."""
    safe_title = new_title.replace("<", "").replace(">", "").strip()[:100]

    if re.search(r"<title>.*?</title>", html, flags=re.DOTALL | re.IGNORECASE):
        html = re.sub(
            r"<title>.*?</title>",
    ...
    if re.search(r'<meta\s+property=["\']og:title["\']', html, flags=re.IGNORECASE):
        html = re.sub(
...
async def _do_rename_title(update: Update, uid: str, proj: str, new_title: str):
    await _typing(update)
    local = get_project_files(uid, proj) or get_project_files_db(uid, proj)
    if not local:
        await update.message.reply_text("⚠️ لم أجد ملفات هذا المشروع.")
        return
    ...
    for f in local:
        if f["path"] == "index.html":
            files.append({"path": f["path"], "content": _rename_page_title(f["content"], new_title)})
            found_html = True
        else:
            files.append(f)

    if not found_html:
        await update.message.reply_text("⚠️ لم أجد index.html بهذا المشروع.")
        return

    try:
        build_project({"projectName": proj, "files": files}, uid)
        url = deploy_project(proj, files)
        add_project(uid, proj, url, files)
        log(f"[RENAME_OK] uid={uid} proj={proj} title={new_title[:50]}")
        await update.message.reply_text(
            f"✅ تم تغيير العنوان إلى: {new_title}\n🔗 الرابط نفسه ما تغيّر:\n{url}",
            reply_markup=_project_keyboard(proj, url),
        )
    except Exception as e:
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if uid in RENAME_MODE:
            ...
            await _do_rename_title(update, uid, proj, text)
            return
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "edit":
        ...
        await q.message.reply_text(
            f"📝 أرسل الآن العنوان الجديد لموقع {proj}.\n"
            f"هذا يغيّر فقط العنوان اللي يطلع بمعاينة الرابط، الرابط نفسه ما يتغيّر.\nللإلغاء: /cancel"
        )
    elif action == "del":
